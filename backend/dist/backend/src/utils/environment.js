...
exports.printEnvironmentSummary = printEnvironmentSummary;
...
function validateEnvironment() {
    ...
    simple_logger_1.logger.info(`Validating environment for: ${nodeEnv}`);
    const missingVars = [];
    const invalidVars = [];
    ...
    if (missingVars.length > 0 || invalidVars.length > 0) {
        ...
        simple_logger_1.logger.error('Environment validation failed', { missingVars, invalidVars });
    ...
    Object.keys(process.env).forEach(key => {
        if (process.env[key] !== undefined) {
            environmentConfig[key] = process.env[key];
        }
    });
    // Apply defaults
    ...
    simple_logger_1.logger.info('✅ Environment validation completed successfully');
}
/**
...
function printEnvironmentSummary() {
    ...
    simple_logger_1.logger.info('Environment configuration:', summary);
}
